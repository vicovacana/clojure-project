(ns maze-test
  (:require [midje.sweet :refer :all]
            ...
            [maze-solver :refer [solve-a-star]]))
...
(fact "A* - success resolved maze"
      (let [maze-data (generate-maze 10 10)
            grid (:grid maze-data)
            start (:start-cell maze-data)
            end (:end-cell maze-data)
            solution (solve-a-star grid start end)
            path (:path solution)]
